...
class ExploreCategories extends Component {
  ...
  render() {
    ...
    let categoriesList = categories.map(category => {
      return (
        <Box inlineBlock>
          <Card className="card" key={category.id} border shadow maxW="sm" style={{width:"314px", height:"200px", marginRight:"15px", marginBottom:"15px", backgroundColor:"#F1F5F8", textAlign:"Center"}}>
            <div >
              <img src={category.image} />
            </div>
          </Card>
          <div style={{fontSize: "16px", fontWeight:"600", color:"#2e3e48", marginBottom: "1rem"}}>{category.name}</div>
        </Box>
      )
    })
